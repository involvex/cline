...
export async function goProtoc(outDir, protoFiles) {
	...
	const goProtocCommand = [
		...
		`--go_out="${outDir}"`,
		`--go_opt=module=github.com/cline/cli`,
		`--go-grpc_out="${outDir}"`,
		`--go-grpc_opt=module=github.com/cline/cli`,
		...protoFiles,
	].join(" ")

	try {
		console.log(chalk.cyan(`Generating Go code in ${outDir}...`))
		console.log(chalk.gray(`Running: ${goProtocCommand}`))

		// Get Go bin path and add to PATH
		let goBinPath = ""
		try {
			goBinPath = execSync("go env GOBIN", { encoding: "utf8" }).trim()
			if (!goBinPath) {
				goBinPath = execSync("go env GOPATH", { encoding: "utf8" }).trim() + "/bin"
			}
		} catch (e) {
			// Fallback to default
			goBinPath = process.env.HOME + "/go/bin"
		}

		const env = { ...process.env }
		if (goBinPath && env.PATH && !env.PATH.includes(goBinPath)) {
			const separator = process.platform === "win32" ? ";" : ":"
			env.PATH = goBinPath + separator + env.PATH
		}

		execSync(goProtocCommand, { stdio: "inherit", env })
	} catch (error) {
		console.error(chalk.red("Error generating Go code:"), error)
		console.error(chalk.red("Command was:"), goProtocCommand)
	...
	// Don't generate a separate go.mod for the generated code
	// await generateGoMod()
...
func (sc *${capitalizedServiceName}Client) ${capitalizedMethodName}(ctx context.Context, req ${requestType}) (${responseType}, error) {
	...
	return resp, nil
		...
		const clineImport = needsClineImport ? '	cline "github.com/cline/cli/pkg/generated/cline"\n' : ""
